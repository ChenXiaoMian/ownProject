...
/***/ function(module, exports, __webpack_require__) {
	...
	function validate(selector) {
	    ...
	    $eles.forEach(function (ele) {
	        ...
	        var $patterns = $form.find('[pattern]');
	        ...
	        for (var j = 0, len = $patterns.length; j < len; ++j) {
	            var $pattern = $patterns.eq(j),
	                errorMsg = _validate($pattern, $form, options.regexp),
	                error = { ele: $pattern[0], msg: errorMsg };
	            if($pattern.attr("required")==true || $pattern.val()!=''){
		            if (errorMsg) {
		                if (!callback(error)) _showErrorMsg(error);
		                return;
		            }
